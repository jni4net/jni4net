...
namespace com.jni4net.proxygen.Utils
{
    public static class ReflectionUtils
    {
        ...
        public static bool DetectIsClr(Class jvm, Type clr, IKnownTypes knownTypes)
        {
            bool isClr;
            if (clr.IsInterface
                    ? IsProxyAnnotatedInterface(clr, knownTypes)
                    : IsProxyAnnotatedType(clr, knownTypes))
            {
                isClr = false;
            }
        ...
        public static bool IsProxyAnnotatedInterface(Type type, IKnownTypes knownTypes)
        {
        ...
        public static bool IsProxyAnnotatedType(Type type, IKnownTypes knownTypes)
        {
            if (type.IsInterface) return false;
            foreach (var customAttributeData in type.__GetCustomAttributes(null, false))
            {
